#!/usr/bin/env python3
"""GraphSkillEvo eval-only: run a single skill on a dataset without training.

Usage
-----
    python scripts/eval_only.py \
        --config configs/spreadsheetbench/default.yaml \
        --skill graphskillevo/initial_skills/spreadsheetbench/init_skill.md \
        --split_dir /path/to/split \
        --out_root outputs/eval_skill0

All YAML keys can be overridden from the CLI, same as the training entrypoint.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="GraphSkillEvo eval-only")
    ...
    p.add_argument("--backend", type=str,
                   choices=["azure_openai", "openai_chat", "codex", "codex_exec"])
...
def main() -> None:
    ...
    from graphskillevo.config import load_config as _load, flatten_config, is_structured
    ...
    from graphskillevo.model.common import default_model_for_backend, normalize_backend_name
    from graphskillevo.compat_utils import compute_score
    ...
    if cli:
        if structured:
            from graphskillevo.config import apply_overrides
            _MAP = {
    ...
    if cfg.get("optimizer_backend") != "openai_chat":
        raise ValueError("optimizer_backend must be openai_chat")
    if cfg.get("target_backend") not in {"openai_chat", "codex_exec"}:
