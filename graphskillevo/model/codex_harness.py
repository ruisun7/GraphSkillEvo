...
from graphskillevo.model.backend_config import (
...
def render_skill_md(
    skill_content: str,
    *,
    name: str = "graphskillevo-target",
    description: str = "Dynamic GraphSkillEvo skill for the current benchmark task.",
    preamble: str = "",
) -> str:
...
def prepare_workspace(
    *,
    ...
    os.makedirs(os.path.join(work_dir, ".agents", "skills", "graphskillevo-target"), exist_ok=True)

    skill_path = os.path.join(work_dir, ".agents", "skills", "graphskillevo-target", "SKILL.md")
...
_DENIED_DATA_DIR_NAMES: set[str] = set()
...
def _exec_prompt(prompt: str, *, allow_file_edits: bool = False) -> str:
    ...
    return (
        ...
        ".agents/skills/graphskillevo-target/SKILL.md before answering. "
        "If ATTACHMENTS.md exists, read it and inspect the listed local files. "
        "Do not call a Skill tool; the GraphSkillEvo guidance is a local markdown file. "
...
def _retry_prompt(prompt: str, attempt: int) -> str:
    ...
    return (
        ...
        ".agents/skills/graphskillevo-target/SKILL.md. If ATTACHMENTS.md exists, use the listed files. "
        "Then produce the final answer inside <answer>...</answer>."
    )


def _normalize_target_exec_prompt(prompt: str) -> str:
    """Normalize target prompts for local file-based guidance."""
    text = prompt or ""
    replacements = {
        "Use the `graphskillevo-target` skill available in this workspace.": (
            "Read `.agents/skills/graphskillevo-target/SKILL.md` directly; do not call a Skill tool."
        ),
        "- Use the local `graphskillevo-target` skill before writing code.": (
            "- Read `.agents/skills/graphskillevo-target/SKILL.md` before writing code; do not call a Skill tool."
        ),
    }
...
def _run_codex_sdk_exec(
    *,
    ...
    async def _query() -> tuple[str, str]:
        ...
        sandbox_value = str(config.get("sandbox") or "workspace-write").strip().lower()
        sandbox_map = {
        ...
        sandbox_mode = sandbox_map.get(sandbox_value, Sandbox.workspace_write)
        approval_mode = approval_map.get(approval_value, ApprovalMode.deny_all)
        effort_map = {
        ...
        thread_options: dict[str, Any] = {
            "cwd": work_dir,
            "sandbox": getattr(sandbox_mode, "value", str(sandbox_mode)),
            "approval_mode": getattr(approval_mode, "value", str(approval_mode)),
        }
        if model:
        ...
        if reasoning_effort and reasoning_effort != "none":
            run_options["effort"] = effort_map.get(reasoning_effort.lower(), ReasoningEffort.medium)
        ...
        try:
            ...
            turn = await asyncio.to_thread(thread.run, prompt, **run_options)
        finally:
            codex.close()
