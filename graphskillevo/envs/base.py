"""GraphSkillEvo environment adapter — abstract interface.

To connect GraphSkillEvo to a new environment (benchmark, simulator, etc.),
...
from graphskillevo.datasets.base import BaseDataLoader, BatchSpec


class EnvAdapter(ABC):
    """Abstract adapter for connecting GraphSkillEvo to any environment.

    Subclasses must implement all abstract methods. The GraphSkillEvo trainer
    ...
    def rollout(
        ...
        **kwargs,
        ...
        -------
        list[dict]
            Each dict must have ``"id"`` (str), ``"hard"`` (0/1), and
            ``"soft"`` (float 0-1). It may include env-specific fields.
        """
