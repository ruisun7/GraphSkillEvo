"""Prompt loading utilities for GraphSkillEvo.

Prompts are stored as ``.md`` files and loaded at runtime:

- **Generic** prompts live in ``graphskillevo/prompts/*.md``
- **Env-specific** prompts live in ``graphskillevo/envs/<env>/prompts/*.md``
...
def load_prompt(name: str, env: str | None = None) -> str:
    ...
    Lookup order:
      1. ``graphskillevo/envs/{env}/prompts/{name}.md``  (if *env* given)
      2. ``graphskillevo/prompts/{name}.md``              (generic default)
    ...
    searched = []
    if env is not None:
        searched.append(os.path.join("graphskillevo/envs", env, "prompts", f"{name}.md"))
    searched.append(f"graphskillevo/prompts/{name}.md")
